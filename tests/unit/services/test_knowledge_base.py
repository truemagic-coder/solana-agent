    ...
    with patch(
        ...
        instance.aget_text_embedding_batch = AsyncMock(
            side_effect=lambda texts, **kwargs: [
                [0.1 + i * 0.01] * 1536 for i in range(len(texts))
            ]
        )
...
def mock_semantic_splitter():
    ...
    with patch(
        ...
        instance.get_nodes_from_documents = MagicMock(
            return_value=[
                TextNode(text="Chunk 1", id_="chunk_node_1", metadata={"some": "meta"}),
                TextNode(
                    text="Chunk 2", id_="chunk_node_2", metadata={"other": "meta"}
                ),
            ]
        )
...
def mock_pypdf_reader():
    with patch(
        "solana_agent.services.knowledge_base.pypdf.PdfReader"
...
class TestKnowledgeBaseServiceInitialization:
    ...
    def test_init_successful_custom_args(
        ...
        mock_mongodb_adapter.collection_exists.assert_called_once_with("custom_coll")
    ...
    def test_init_missing_api_key_raises_error(
        ...
        with pytest.raises(ValueError, match="OpenAI API key not provided"):
            KnowledgeBaseService(
                mock_pinecone_adapter, mock_mongodb_adapter, openai_api_key=None
            )  # Pass None
    ...
    def test_init_unknown_model_uses_pinecone_dim(
        self,
        mock_pinecone_adapter,
        mock_mongodb_adapter,
        mock_openai_embedding,
...
class TestKnowledgeBaseServiceAddDocument:
    async def test_add_document_success_auto_id(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
    ...
    async def test_add_document_success_provided_id_namespace_rerank(
        self,
        kb_service_custom,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        doc_id = await service.add_document(
            text, metadata, document_id=provided_id, namespace=namespace
        )
    ...
    async def test_add_document_mongo_error(
        ...
        service = kb_service_default
        mock_mongodb_adapter.insert_one.side_effect = Exception("Mongo write failed")
        embed_model_instance = service.semantic_splitter.embed_model
...
class TestKnowledgeBaseServiceAddPdfDocument:
    async def test_add_pdf_document_bytes_success(
        ...
        expected_chunk_embeddings = [[0.1] * 1536, [0.11] * 1536]  # From batch mock
        ...
        # Check arg type
        assert isinstance(mock_asyncio_to_thread.call_args[0][1][0], LlamaDocument)
    ...
    async def test_add_pdf_document_path_success(
        ...
        expected_parent_id = str(mock_uuid.return_value)
        expected_chunk_texts = ["Chunk 1", "Chunk 2"]
        expected_chunk_embeddings = [[0.1] * 3072, [0.11] * 3072]  # Default model
    ...
    async def test_add_pdf_document_pdf_read_error(
        self, kb_service_default, mock_pypdf_reader
    ):
        mock_pypdf_reader.side_effect = pypdf.errors.PdfReadError("Invalid PDF")
    ...
    async def test_add_pdf_document_chunk_embedding_error(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pypdf_reader,
...
class TestKnowledgeBaseServiceQuery:
    async def test_query_success_no_rerank(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
        ...
        pinecone_res = [
            {
                "id": "doc1",
                "score": 0.9,
                "metadata": {"document_id": "doc1", "source": "s1", "is_chunk": False},
            },
            {
                "id": "pdf1_chunk_0",
                "score": 0.8,
        ...
        results = await service.query(
            query, top_k=2, include_content=True, include_metadata=True
        )

        embed_model_instance.aget_query_embedding.assert_awaited_once_with(query)
    ...
    async def test_query_success_with_rerank(
        self, kb_service_custom, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_custom  # Reranking enabled
        ...
        pinecone_res = [
            {
                ...
                "score": 0.95,
                "metadata": {
                    "document_id": "pdf1_chunk_1",
                    "parent_document_id": "pdf1",
                    "is_chunk": True,
                    "chunk_index": 1,
                    rerank_field: "Reranked Chunk 1 Text",
                },
            },
            {
                "id": "doc2",
                ...
                "metadata": {
                    ...
                    "source": "s2",
                    "is_chunk": False,
                    rerank_field: "Reranked Doc2 Text",
                },
            },
        ]
        mock_pinecone_adapter.query.return_value = pinecone_res
    ...
    async def test_query_with_filter_namespace_no_content(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
        ...
        pinecone_res = [
            {
                "id": "doc3",
                "score": 0.7,
                "metadata": {"document_id": "doc3", "is_chunk": False},
            }
        ]
        mock_pinecone_adapter.query.return_value = pinecone_res
    ...
    async def test_query_mongo_doc_not_found(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
        ...
        pinecone_res = [
            {
                "id": "doc_missing",
                "score": 0.6,
                "metadata": {"document_id": "doc_missing", "is_chunk": False},
            }
        ]
        mock_pinecone_adapter.query.return_value = pinecone_res
    ...
    async def test_query_pinecone_error(
        self, kb_service_default, mock_pinecone_adapter
    ):
        service = kb_service_default
    ...
    async def test_query_mongo_find_error(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter, capsys
    ):
        service = kb_service_default
        pinecone_res = [
            {
                "id": "doc1",
                "score": 0.9,
                "metadata": {"document_id": "doc1", "is_chunk": False},
            }
        ]
        mock_pinecone_adapter.query.return_value = pinecone_res
    ...
    async def test_query_no_results_from_pinecone(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
...
class TestKnowledgeBaseServiceDeleteDocument:
    async def test_delete_document_pdf_with_chunks(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
        ...
        mock_mongodb_adapter.delete_many.return_value = MagicMock(deleted_count=3)
        ...
        mock_mongodb_adapter.find.assert_called_once_with(
            service.collection,
            {"$or": [{"document_id": parent_id}, {"parent_document_id": parent_id}]},
        )
        ...
        assert sorted(call_kwargs.get("ids", [])) == sorted(
            expected_pinecone_ids
        )  # NEW Check IDs ignoring order

        # Check Mongo delete call (using IDs found in Mongo)
        mock_mongodb_adapter.delete_many.assert_called_once_with(
            service.collection, {"document_id": {"$in": expected_pinecone_ids}}
        )

    async def test_delete_document_plain_text(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
    ...
    async def test_delete_document_mongo_find_error(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter, capsys
    ):
        service = kb_service_default
    ...
    async def test_delete_document_pinecone_error(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
    ...
    async def test_delete_document_mongo_delete_error(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
        ...
        mongo_find_results = [{"document_id": doc_id}]
        mock_mongodb_adapter.find.return_value = mongo_find_results
        mock_mongodb_adapter.delete_many.side_effect = Exception("Mongo delete crashed")
    ...
    async def test_delete_document_not_found_anywhere(
        self, kb_service_default, mock_mongodb_adapter, mock_pinecone_adapter
    ):
        service = kb_service_default
        ...
        mock_mongodb_adapter.delete_many.return_value = MagicMock(deleted_count=0)
...
class TestKnowledgeBaseServiceUpdateDocument:
    ...
    async def test_update_document_metadata_only(
        ...
        new_metadata = {"source": "updated_source", "tags": ["tagB"], "new_field": True}
    ...
    async def test_update_document_text_only_rerank_off(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        updated = await service.update_document(
            doc_id, text=new_text, namespace="update-ns"
        )
        ...
        embed_model_instance.aget_text_embedding.assert_awaited_once_with(new_text)
    ...
    async def test_update_document_text_and_metadata_rerank_on(
        self,
        kb_service_custom,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        updated = await service.update_document(
            doc_id, text=new_text, metadata=new_metadata
        )
    ...
    async def test_update_document_not_found(
        self, kb_service_default, mock_mongodb_adapter
    ):
        service = kb_service_default
        doc_id = "does-not-exist"
    ...
    async def test_update_document_is_chunk_error(
        self, kb_service_default, mock_mongodb_adapter, existing_doc
    ):
        service = kb_service_default
        doc_id = existing_doc["document_id"]
    ...
    async def test_update_document_pdf_text_error(
        self, kb_service_default, mock_mongodb_adapter, existing_doc
    ):
        service = kb_service_default
        doc_id = existing_doc["document_id"]
    ...
    async def test_update_document_no_changes(
        ...
        mock_mongodb_adapter.update_one.return_value = MagicMock(modified_count=0)
        expected_update_time = mock_datetime.now.return_value
    ...
    async def test_update_document_mongo_update_fails(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        doc_id = existing_doc["document_id"]
        mock_mongodb_adapter.find_one.return_value = existing_doc
        mock_mongodb_adapter.update_one.side_effect = Exception("Mongo update failed")
    ...
    async def test_update_document_text_embedding_fails(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        embed_model_instance = service.semantic_splitter.embed_model
        embed_model_instance.aget_text_embedding.side_effect = Exception("Embed failed")
    ...
    async def test_update_document_text_pinecone_fails(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        mock_pinecone_adapter.upsert.side_effect = Exception("Pinecone update failed")
...
class TestKnowledgeBaseServiceAddDocumentsBatch:
    ...
    async def test_add_documents_batch_multiple_batches(
        self,
        kb_service_custom,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
        ...
        embed_model_instance.aget_text_embedding_batch.side_effect = [
            expected_embeddings_batch1,
            expected_embeddings_batch2,
        ]

        added_ids = await service.add_documents_batch(
            docs_to_add, batch_size=2, namespace="multi-batch"
        )
        ...
        assert (
            ...
            in first_pinecone_call[1]["vectors"][0]["metadata"]
        )
        assert (
            first_pinecone_call[1]["vectors"][0]["metadata"][
                service.pinecone.rerank_text_field
            ]
            == "Doc 1 text"
        )
        ...
        assert (
            service.pinecone.rerank_text_field
            in second_pinecone_call[1]["vectors"][0]["metadata"]
        )
        assert (
            second_pinecone_call[1]["vectors"][0]["metadata"][
                service.pinecone.rerank_text_field
            ]
            == "Doc 3 text"
        )
    ...
    async def test_add_documents_batch_mongo_error_skips_batch(
        self,
        kb_service_default,
        mock_mongodb_adapter,
        mock_pinecone_adapter,
    ...
    async def test_add_documents_batch_pinecone_error_logged(
        ...
        mock_pinecone_adapter.upsert.side_effect = Exception("Pinecone batch failed")
...
class TestKnowledgeBaseServiceGetFullDocument:
    async def test_get_full_document_success(
        self, kb_service_default, mock_mongodb_adapter
    ):
        service = kb_service_default
        doc_id = "get-me-123"
    ...
    async def test_get_full_document_not_found(
        self, kb_service_default, mock_mongodb_adapter
    ):
        service = kb_service_default
        doc_id = "not-found-id"
    ...
    async def test_get_full_document_mongo_error(
        self, kb_service_default, mock_mongodb_adapter, capsys
    ):
        service = kb_service_default
        doc_id = "mongo-error-id"
        mock_mongodb_adapter.find_one.side_effect = Exception("Mongo find_one failed")
