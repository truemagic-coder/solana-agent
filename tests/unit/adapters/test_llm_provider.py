...
@pytest.mark.asyncio
async def test_parse_structured_output_fallback(openai_adapter):
    """Test fallback parsing when beta API fails."""
    # Setup beta API to fail
    openai_adapter.mock_client.beta.chat.completions.parse.side_effect = Exception(
        "Not supported")

    # Setup regular API response
    json_response = '{"name": "fallback", "value": 99, "items": ["a", "b", "c"]}'
    mock_completion = MockCompletion(json_response)
    openai_adapter.mock_client.chat.completions.create.return_value = mock_completion
    ...
    openai_adapter.mock_client.chat.completions.create.assert_called_once()
    assert openai_adapter.mock_client.chat.completions.create.call_args[1]["response_format"] == {
        "type": "json_object"}

    # Should return the parsed model from JSON
...
@pytest.mark.asyncio
async def test_parse_structured_output_double_failure(openai_adapter):
    """Test returning default model when both parsing methods fail."""
    # Setup beta API to fail
    openai_adapter.mock_client.beta.chat.completions.parse.side_effect = Exception(
        "Not supported")

    # Setup regular API to return invalid JSON
    mock_completion = MockCompletion("This is not JSON")
    openai_adapter.mock_client.chat.completions.create.return_value = mock_completion
    ...
    # Should return default model instance
    assert isinstance(result, TestModel)
    assert result.name == ""  # Default value
    assert result.value == 0  # Default value
    assert result.items == []  # Default value


@pytest.mark.asyncio
async def test_parse_structured_output_custom_model(openai_adapter):
    """Test parsing with a custom Pydantic model."""
    ...
    # Setup beta API to fail so we test the fallback
    openai_adapter.mock_client.beta.chat.completions.parse.side_effect = Exception(
        "Not supported")

    # Setup regular API response
    json_response = '{"title": "Custom Title", "count": 5, "active": true}'
    mock_completion = MockCompletion(json_response)
    openai_adapter.mock_client.chat.completions.create.return_value = mock_completion

    # Call the method with custom model
    result = await openai_adapter.parse_structured_output(
        prompt="Generate custom data",
        system_prompt="Return custom model",
